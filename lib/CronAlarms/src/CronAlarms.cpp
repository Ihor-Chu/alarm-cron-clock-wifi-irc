...
{
  //Serial.print("isAllocated(");Serial.print(ID);Serial.print(")=");Serial.println((ID < dtNBR_ALARMS && Alarm[ID].onTickHandler != NULL));
...
void CronClass::serviceAlarms()
{
  if (!isServicing) {
    ...
    for (servicedCronId = 0; servicedCronId < dtNBR_ALARMS; servicedCronId++) {
      if (Alarm[servicedCronId].isEnabled && this->now() >= Alarm[servicedCronId].nextTrigger) {
...
CronID_t CronClass::create(char * cronstring, OnTick_t onTickHandler, bool isOneShot)
{
  for (uint8_t id = 0; id < dtNBR_ALARMS; id++) {
    if (!isAllocated(id)) {
      ...
      Serial.print("Cronstring='" + String(cronstring) + "' "); Serial.println(err);
      if (err) {
        memset(&(Alarm[id].expr), 0, sizeof(Alarm[id].expr));
